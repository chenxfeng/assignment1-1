...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        """TODO: Your code here"""
        assert len(input_vals) == 2
        return np.matmul(np.transpose(input_vals[0]) if node.matmul_attr_trans_A \
                                                    else input_vals[0], \
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if isinstance(node.op, PlaceholderOp):
                continue
            elif isinstance(node.op, AddOp):
                op = add_op
            elif isinstance(node.op, MulOp):
                op = mul_op
            elif isinstance(node.op, AddByConstOp):
                op = add_byconst_op
            elif isinstance(node.op, MulByConstOp):
                op = mul_byconst_op
            elif isinstance(node.op, MatMulOp):
                op = matmul_op
            elif isinstance(node.op, OnesLikeOp):
                op = oneslike_op
    ...
    for node in reverse_topo_order:
        ...
        for n in node.inputs:
            if n in node_to_output_grad:
                node_to_output_grad[n] = node_to_output_grad[n] + node_to_output_grads_list[node][i]
            else:
                node_to_output_grad[n] = node_to_output_grads_list[node][i]
            i = i + 1
            # print(node_to_output_grad[n].name)
        #wrong# [node_to_output_grad[n] for n in node.inputs] = node_to_output_grads_list[node]
